...
[![GitHub Actions](https://img.shields.io/github/actions/workflow/status/phoenix1331/laravel-env-audit/tests.yml?style=flat-square)](https://github.com/phoenix1331/laravel-env-audit/actions)
...
A third, more serious version: `.env.example` is committed to git and sometimes copy-pasted from a real `.env` file — a surprisingly common place for a genuine secret to end up in a public repository, disguised as a placeholder.
...
### 1. Direct env() usage outside config/ — Error (default)
...
### 2. Possible secret in .env.example — Error (default)
...
### 3. Missing from .env.example — Warning (default)
...
### 4. Unused in .env.example — Info (default)
...
Both bypasses are recorded in the report's exclusions section with their reasons. The count of active bypasses is reported as its own metric — to stop the isolation score being inflated by liberal use of the bypass instead of actually fixing call sites.
...
- **v2:** Optional git-history mode layering the secret heuristic across recent commits (clearly separated from dedicated tools like Gitleaks)
- **v2/v3:** A shared rendering/config-publishing library extracted from both `laravel-auth-audit` and `laravel-env-audit`
- **Stretch:** A GitHub Action wrapper, matching the sibling package's stretch goal
